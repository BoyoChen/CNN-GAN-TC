...
# ------------
# Preprocess visible channel with tf.image.per_image_standardization.
# ------------
...
class Model(tf.keras.Model):
    ...
    def image_preprocessing(self, image, training):
        ...
        VIS = image[:, :, :, 2:3]
        standardized_VIS = tf.image.per_image_standardization(VIS)
        IR1_PMW_VIS = tf.concat([IR1_PMW, standardized_VIS], axis=-1)
        normalized_image = self.batch_norm(IR1_PMW_VIS, training=training)
